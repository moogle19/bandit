defmodule Bandit.WebSocket.Connection do
  ...
  def handle_frame(frame, socket, %{fragment_frame: nil} = connection) do
    ...
    case frame do
      ...
      %Frame.Text{fin: true, compressed: true} ->
        do_inflate(frame, socket, connection)

      %Frame.Text{fin: true, data: data} ->
        if !Keyword.get(connection.opts, :validate_text_frames, true) || String.valid?(data) do
          connection.websock.handle_in({data, opcode: :text}, connection.websock_state)
          |> handle_continutation(socket, connection)
        else
          do_error(1007, "Received non UTF-8 text frame (RFC6455§8.1)", socket, connection)
        end

      %Frame.Text{fin: false} ->
        {:continue, %{connection | fragment_frame: frame}}

      %Frame.Binary{fin: true, compressed: true} ->
        do_inflate(frame, socket, connection)

      %Frame.Binary{fin: true, data: data} ->
        connection.websock.handle_in({data, opcode: :binary}, connection.websock_state)
        |> handle_continutation(socket, connection)

      %Frame.Binary{fin: false} ->
        {:continue, %{connection | fragment_frame: frame}}
  ...
  def handle_frame(frame, socket, %__MODULE__{fragment_frame: fragment_frame} = connection)
      when not is_nil(fragment_frame) do
    connection = do_recv_metrics(frame, connection)

    case frame do
      %Frame.Continuation{fin: true} = frame ->
        data = IO.iodata_to_binary([fragment_frame.data | frame.data])
        frame = %{fragment_frame | fin: true, data: data}
        handle_frame(frame, socket, %{connection | fragment_frame: nil})

      %Frame.Continuation{fin: false} = frame ->
        data = [fragment_frame.data | frame.data]
        frame = %{fragment_frame | fin: true, data: data}
        {:continue, %{connection | fragment_frame: frame}}
  ...
  defp handle_control_frame(frame, socket, %__MODULE__{} = connection) do
    case frame do
      ...
      %Frame.Ping{data: data} ->
        connection =
          Socket.send_frame(socket, {:pong, data}, false)
          |> do_send_metrics(connection)

        if function_exported?(connection.websock, :handle_control, 2) do
          connection.websock.handle_control({data, opcode: :ping}, connection.websock_state)
          |> handle_continutation(socket, connection)
        else
          {:continue, connection}
        end

      %Frame.Pong{data: data} ->
        if function_exported?(connection.websock, :handle_control, 2) do
          connection.websock.handle_control({data, opcode: :pong}, connection.websock_state)
          |> handle_continutation(socket, connection)
        else
          {:continue, connection}
        end
  ...
  defp do_recv_metrics(frame, %__MODULE__{metrics: metrics} = connection) do
    metrics =
      Bandit.WebSocket.Frame.recv_metrics(frame)
      |> Enum.reduce(metrics, fn {key, value}, metrics ->
  ...
  defp do_send_metrics(metrics, %__MODULE__{} = connection) do
    metrics =
      metrics
  ...
  def handle_info(msg, socket, %__MODULE__{} = connection) do
  ...
  defp do_stop(code, reason, socket, %__MODULE__{websock: websock} = connection) do
    if connection.state == :open do
      if function_exported?(websock, :terminate, 2) do
        websock.terminate(reason, connection.websock_state)
      end
  ...
  defp do_error(code, reason, socket, %__MODULE__{websock: websock} = connection) do
    if connection.state == :open do
      if function_exported?(websock, :terminate, 2) do
        websock.terminate(maybe_wrap_reason(reason), connection.websock_state)
      end
  ...
  defp do_deflate({opcode, data} = msg, socket, %__MODULE__{} = connection)
  ...
  defp do_inflate(frame, socket, %__MODULE__{} = connection) do
